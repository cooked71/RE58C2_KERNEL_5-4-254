...
TRACE_EVENT(sched_pelt_se,
	...
	TP_printk("cpu=%d path=%s comm=%s pid=%d load=%lu " RBL_LOAD_STR
		  "=%lu util=%lu update_time=%llu",  __entry->cpu, __entry->path,
...
TRACE_EVENT(sched_util_est_se,
	...
	TP_fast_assign(
		...
		strlcpy(__entry->comm, comm, TASK_COMM_LEN);
		__entry->pid		= pid;
		__entry->enqueued	= avg->util_est.enqueued & 0x7fffffff;
